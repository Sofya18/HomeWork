...
Console.Clear();
Console.Write("Введите размер 1-ой матрицы: ");
int[] sizeA = Console.ReadLine().Split(" ").Select(x => int.Parse(x)).ToArray();
int[,] matrixA = new int[sizeA[0], sizeA[1]];
Console.Write("Введите размер 2-ой матрицы: ");
int[] sizeB = Console.ReadLine().Split(" ").Select(x => int.Parse(x)).ToArray();
int[,] matrixB = new int[sizeB[0], sizeB[1]];
if (sizeA[1] != sizeB[0])
{
    Console.WriteLine("ОШИБКА! Количество столбцов первой матрицы не совпадает с количеством строк во второй!");
    return;
}
InputMatrix(matrixA);
InputMatrix(matrixB);
Console.WriteLine("1-я исходная матрица:");
PrintMatrix(matrixA);
Console.WriteLine("2-я исходная матрица:");
PrintMatrix(matrixB);
